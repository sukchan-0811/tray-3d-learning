...
def train(params):
    ...
    if params.category == 'tray':
        train_dataset = Tray3d('../data', 'train')
        val_dataset = Tray3d('../data', 'valid')
    else:
    ...
    step = len(train_dataset) // params.log_frequency
        ...
        for i, (p, c) in enumerate(train_dataloader):            
            ...
            if (i + 1) % step == 0:
                log(log_fd, "Training Epoch [{:03d}/{:03d}] - Iteration [{:03d}/{:03d}]: coarse loss = {:.6f}, dense l1 cd = {:.6f}, total loss = {:.6f}"
                    .format(epoch, params.epochs, i + 1, len(train_dataset), loss1.item() * 1e3, loss2.item() * 1e3, loss.item() * 1e3))
